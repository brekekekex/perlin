...
__global__
void perlin_noise_fill(double *noise_map, unsigned int w, unsigned int h, unsigned int s)
{
	int num_elems = w * h; 
	...
	for (int i = index; i < num_elems; i += stride) {
		...
		BA = p[B];
		BB = p[B + 1];
		noise_map[i] = linterp(y_fade, linterp(x_fade, grad(p[AA], x, y), grad(p[BA], x - 1, y)), linterp(x_fade, grad(p[AB], x, y - 1), grad(p[BB], x - 1, y - 1)));
	}
}
...
int main(void)
{
	...
	// run kernel
	perlin_fill_heights<<<32, 256>>>(d_noise, p->width, p->height, p->samples);	
